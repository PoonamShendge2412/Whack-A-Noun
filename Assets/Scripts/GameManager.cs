...
public class GameManager : MonoBehaviour 
{
    ...
    public TextAsset jsonFile; // Drag `words.json` here
    ...
    public WordEntry GetRandomWord()
    {
        if (wordList.Count == 0)
        {
            Debug.LogWarning("No more words left!");
            return null;
        }

        int index = Random.Range(0, wordList.Count);
        WordEntry selectedWord = wordList[index];
        wordList.RemoveAt(index); // Remove so it's not reused
        return selectedWord;
